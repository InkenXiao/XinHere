...
// + 3 核心技能卡（报告类→AI 意图识别进对话；信息填报→模版选择）+ 知识库/模型下拉（值对：label 展示 / value 传参）
// 发送/点「新对话」→ 进入三栏工作台视图
...
export default function HeroHome() {
  ...
  // 技能卡点击：报告类 → AI 意图识别（发引导 prompt 进对话）；信息填报 → 模版选择 Modal
  const onSkill = (key: string) => {
    const prompt = SKILL_PROMPTS[key]
    if (prompt) void enterChat(prompt)
    else useUiStore.getState().openTemplateModal(key)
  }
