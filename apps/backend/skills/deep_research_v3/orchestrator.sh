...
# V3.1.1: 修复 Windows(Git Bash+Windows Python) 下状态管理失败；
#         修复 reports/ 不存在时 --self-check 静默中断；主题含引号时 JSON 转义
# ============================================================
...
check_dependencies() {
    ...
    if ! command -v jq &>/dev/null && ! command -v python3 &>/dev/null && ! command -v python &>/dev/null; then
        missing+=("jq 或 python3/python")
    fi
    if [[ ${#missing[@]} -gt 0 ]]; then
        echo -e "${RED}⛔ 缺少依赖: ${missing[*]}${NC}"
        echo "请安装 jq，或确保 python3 / python 可用"
        exit 1
    fi
    # 选择可用的 Python（优先 python3，回退 python；Windows 常见只有 python）
...
# --- 路径转换：Git Bash 的 /d/... 路径 Windows Python 打不开，转成 D:/... ---
to_py_path() {
...
5. 生成同名 PDF：`reports/YYYY-MM-DD-{主题}.pdf`（优先用环境内置文档导出能力，其次 pandoc/xelatex 并指定中文字体；全部不可用则告知用户并说明原因）
6. 发送摘要给用户
...
| 12 | 报告(MD+PDF)保存在正确路径且同名 | 检查 reports/ 目录是否有当天 .md 及同名 .pdf |
...
init_session() {
    ...
    # 转义主题中的特殊字符，防止破坏 JSON
    local topic_json="${TOPIC//\\/\\\\}"
...
complete_step() {
    ...
    if command -v jq &>/dev/null; then
        ...
        local py_file
        py_file="$(to_py_path "$state_file")"
        "$PY_BIN" - "$py_file" "$step" <<'PYEOF'
import json, sys
...
advance_step() {
    ...
    if command -v jq &>/dev/null; then
        ...
        local py_file
        py_file="$(to_py_path "$state_file")"
        "$PY_BIN" - "$py_file" "$next_step" <<'PYEOF'
import json, sys
path, next_step = sys.argv[1], sys.argv[2]
with open(path, encoding='utf-8') as f:
    state = json.load(f)
...
run_self_check() {
    ...
    # 查找最新的报告文件（|| true 防止 reports/ 不存在时 ls 在 pipefail 下中断脚本）
    ...
    if [[ -z "$LATEST_REPORT" ]]; then
        ...
        # 检查7: PDF 是否已生成（与 MD 同名）
        PDF_REPORT="${LATEST_REPORT%.md}.pdf"
        if [[ -f "$PDF_REPORT" ]]; then
            PDF_SIZE=$(wc -c < "$PDF_REPORT")
            if [[ "$PDF_SIZE" -gt 10000 ]]; then
                echo -e "  #07 PDF 已生成 ($(basename "$PDF_REPORT"), ${PDF_SIZE} bytes)  ${GREEN}✅${NC}"
            else
                echo -e "  #07 PDF 文件过小 (${PDF_SIZE} bytes)，疑似生成失败  ${YELLOW}⚠️${NC}"
            fi
        else
            echo -e "  #07 缺少同名 PDF 文件 (${PDF_REPORT})  ${RED}❌${NC}"
        fi
  ...
  #10 s2 反思校验表是否已输出？                 [ ] ✅ / ❌
  #11 核心数据是否有 3+ 独立来源印证？          [ ] ✅ / ❌
  #12 T3-T4 级信源是否未作为核心论据？          [ ] ✅ / ❌
  #13 纵轴叙事是否为故事体（非流水账）？        [ ] ✅ / ❌
  #14 交汇洞察是否为独立新判断（非缩写）？      [ ] ✅ / ❌
  #15 数据缺口和低置信度结论是否已标注？        [ ] ✅ / ❌
  #16 是否给出了可操作建议？                    [ ] ✅ / ❌
  #17 报告(MD+PDF)是否保存在正确路径且同名？    [ ] ✅ / ❌
CHECKLIST

    echo ""
    echo -e "${RED}⛔ 所有 17 项（自动 7 + 手动 10）必须全部 ✅ 才能交付${NC}"
    ...
    case "${1:-}" in
        ...
        *)
            ...
            if [[ ! -f "${CHECKPOINT_DIR}/session.json" ]]; then
                ...
                load_session
                local py_file
                py_file="$(to_py_path "${CHECKPOINT_DIR}/session.json")"
                CURRENT=$("$PY_BIN" -c "import json,sys; s=json.load(open(sys.argv[1],encoding='utf-8')); print(s.get('current_step',''))" "$py_file")
                if [[ -z "$CURRENT" ]]; then
                    echo -e "${YELLOW}⚠️ 当前没有待执行的步骤。检查是否已完成所有步骤。${NC}"
                    echo "已完成的步骤:"
                    "$PY_BIN" -c "import json,sys; s=json.load(open(sys.argv[1],encoding='utf-8')); print(', '.join(s.get('completed_steps',[])))" "$py_file"
                else
                    show_step "$CURRENT"
                fi
            fi
