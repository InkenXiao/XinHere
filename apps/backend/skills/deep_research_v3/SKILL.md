...
12. **MD+PDF 成对交付**：报告必须同时产出 `reports/YYYY-MM-DD-{主题}.md` 和同名 PDF（`reports/YYYY-MM-DD-{主题}.pdf`）。优先使用环境内置文档导出能力，其次 pandoc/xelatex（指定中文字体）。全部不可用时明确告知用户，不可假装已生成
...
脚本会自动检查 7 项（文件完整性 / 结构 / 推测标注 / 引用 / 模糊表述 / **同名 PDF 是否存在**），然后列出 10 项手动核验清单。
...
| 交付物 | 路径 | 说明 |
|--------|------|------|
| Markdown 报告 | `reports/YYYY-MM-DD-{主题}.md` | 按 report-template.md 的 9 章 + 附录结构 |
| PDF 报告 | `reports/YYYY-MM-DD-{主题}.pdf` | 与 MD 同名，MD 定稿后导出 |
| 执行摘要 | 对话中发送 | 3-5 句核心结论 |
